...
const STORAGE_KEY = "8thsense_theme";
...
function getStoredTheme(fallback: Theme) {
...
export function ThemeToggle({
  ...
  const toggle = useCallback(() => {
    ...
    const fallTimeout = window.setTimeout(() => {
      ...
      setPhase("rising");
      const riseTimeout = window.setTimeout(() => setPhase("idle"), duration + 60);
      timeoutRef.current.push(riseTimeout);
    }, duration);

    timeoutRef.current.push(fallTimeout);
      ...
      <div style={barStyle}>
        ...
        {isAppBar && (
          <div style={{ ...appBarSectionStyle, flex: 1, justifyContent: "flex-end" }}>
            {appBarProps?.userName && <span style={{ fontSize: "0.9rem", opacity: 0.9 }}>{appBarProps.userName}</span>}
