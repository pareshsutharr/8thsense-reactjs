...
export function ParticleTextEffect({
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    do {
      offscreenCtx.font = `${fontWeight} ${fontSize}px Arial`;
      fontSize -= 4;
    } while (
      lines.some((line) => offscreenCtx.measureText(line).width > canvas.width * 0.9) &&
      fontSize > 22
    );

    const actualFontSize = fontSize + 4;
    const blockHeight = actualFontSize * lineHeight * (lines.length - 1);
    lines.forEach((line, index) => {
      offscreenCtx.fillText(
        line,
        canvas.width / 2,
        canvas.height / 2 - blockHeight / 2 + index * actualFontSize * lineHeight,
      );
    });
  ...
  }, [color, fontWeight, maxFontSize, words]);
